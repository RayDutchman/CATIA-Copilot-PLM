/**
 * organization-management-vue 模块入口
 *
 ...
 *   4. 加载组织信息              ── 初始化 organization store
 *   5. createApp + mount        ── 挂载 Vue 3 应用
 */
import { createApp } from 'vue'
import { createPinia } from 'pinia'
import i18n, { mergeModuleStrings } from '../vue-common/i18n.js'
import orgStrings from '../localization/organization-management.js'
...
import '../../app/less/organization-management/style.less'

async function bootstrap() {
  // 追加模块级翻译
  mergeModuleStrings(orgStrings)
  ...
  try {
    ...
    // 步骤 3：加载工作区列表（供 Header 下拉使用）
    await authStore.resolveWorkspaces()

    // 步骤 4：加载组织信息（允许 404 —— 说明用户还没有组织）
    await orgStore.fetchOrganization()
  } catch (err) {
    console.error('[organization-management] bootstrap failed:', err)
