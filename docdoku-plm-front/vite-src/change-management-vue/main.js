...
import changeManagementStrings from '../localization/change-management.js'
import router from './router/index.js'
import App from './App.vue'
import '../../app/less/change-management/style.less'
import { useAppStore } from '../vue-common/store/app.js'
import { useAuthStore } from '../vue-common/store/auth.js'

// 追加 change-management 模块翻译
mergeModuleStrings(changeManagementStrings)
...
appStore.resolveServerProperties()
  ...
  .catch((err) => {
    console.error('[change-management-vue] 初始化失败', err)
    // 重定向到登录页
    const base = window.location.pathname.replace(/change-management-vue.*/, '')
