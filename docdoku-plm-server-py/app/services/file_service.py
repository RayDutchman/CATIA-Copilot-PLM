...
def rename_nativecad(db: Session, ws: str, pn: str, ver: str,
                     iteration: int, old_name: str, new_name: str):
    """重命名 nativecad 文件（vault + BinaryResource + PartIteration 引用）。"""
    old_path = vault.part_nativecad_path(ws, pn, ver, iteration, old_name)
    new_path = vault.part_nativecad_path(ws, pn, ver, iteration, new_name)
    vault.move_file(old_path, new_path)
    old_full = f"{ws}/parts/{pn}/{ver}/{iteration}/nativecad/{old_name}"
    new_full = f"{ws}/parts/{pn}/{ver}/{iteration}/nativecad/{new_name}"
    br = db.query(BinaryResource).filter(
    ...
    if it is not None and it.native_cad_file_fullname == old_full:
        it.native_cad_file_fullname = new_full
    db.flush()


def rename_attached(db: Session, ws: str, pn: str, ver: str,
                    iteration: int, old_name: str, new_name: str):
    """重命名 attached 文件（vault + BinaryResource）。"""
    old_path = vault.part_attached_path(ws, pn, ver, iteration, old_name)
    new_path = vault.part_attached_path(ws, pn, ver, iteration, new_name)
    vault.move_file(old_path, new_path)
    old_full = f"{ws}/parts/{pn}/{ver}/{iteration}/attachedfiles/{old_name}"
    new_full = f"{ws}/parts/{pn}/{ver}/{iteration}/attachedfiles/{new_name}"
    br = db.query(BinaryResource).filter(
        BinaryResource.full_name == old_full).first()
    if br is not None:
        br.full_name = new_full
