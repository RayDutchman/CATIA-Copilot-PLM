...
- [ ] **提取 `_check_is_admin` → `require_admin` Depends**：将 5 个 router 中重复定义的 `_check_is_admin` 统一为一个 FastAPI `Depends` 函数。要求：
...
- [ ] **统一 `_build_acl` / `_get_acl_dict`**：`change_common.py:30`、`product_configurations.py:39`、`part_templates.py:35` 的三个变体统一到 `acl_factory.py` 的 `build_acl_dict(db, acl_id)` 方法。
  - 需要支持 `change_common.py` 额外返回的 `"id"` 和 `"enabled"` 字段（通过可选参数控制）
- [ ] **统一日期格式化**：`product_configurations.py:33`、`change_common.py:65`、`milestones.py:47` 的 `d.strftime("%Y-%m-%dT%H:%M:%S.") + f"{d.microsecond // 1000:03d}Z"` 统一为 `format_iso_date(d)` 工具函数，放入 `app/utils/date_utils.py`（新建或使用已有 `models/util/date_utils.py`）。

### 主 agent Batch 1 收尾
- [ ] 更新所有原引用点（5 个 router 的 `_check_is_admin` + 3 个 router 的 `_build_acl` + 3 个 router 的 `_fmt_date`）改为 import 新位置
- [ ] 删除原文件中的重复定义
- [ ] 部署 + pytest 无新增 fail
- [ ] commit
...
- [ ] 将 `organizations.py` 所有内联 DB 操作（~27 处）迁入 `organization_manager.py`
...
- [ ] 将 `webhooks.py` 所有内联 DB 操作（~13 处）迁入 `webhook_manager.py`
...
- [ ] 将 `share.py` 所有内联 DB 操作（~10 处）迁入 `share_manager.py`
...
- [ ] **document_baselines.py → document_baseline_manager.py**：迁出全部内联 DB 操作（~17 处），包括基线创建/删除/列表的 SQL
- [ ] **product_instances.py → product_instance_manager.py**：迁出 `product_instances.py` 中的内联 DB 逻辑（~22 处），包括 `_replace_instance_attributes`、`update_instance`、`rebase_instance` 的 DB 操作为 service 方法：
...
### 主 agent Batch 2 收尾
- [ ] **`admin.py:261-301` 平台选项 → `platform_options_manager.py`**：迁出 `get_platform_options` 和 `put_platform_options` 的内联 SQL（~8 处），在 service 中新增 `get_options(db)` 和 `set_options(db, data)` 方法。路由层仅保留参数解析+调用+返回。
- [ ] **`products.py:620-682` `_collect_ci_parts` → `cascade_action_manager.py`**：迁出 ~62 行递归采集逻辑（含 5 处 DB 查询），在 service 中新增 `collect_ci_parts(db, ws, ci_id, ps_filter)` 方法。路由层 `cascade_checkout`/`checkin`/`undo` 改为调用此方法。
- [ ] 部署 + smoke：organization CRUD、webhook CRUD、share、document baseline 创建/删除、product instance 更新、product instance 属性 无 500
- [ ] 特别验证 admin.py 平台选项读写一致
- [ ] pytest 无新增 fail
- [ ] commit（分域）
...
- [ ] **`_doc_to_dict` → `document_manager.py:build_document_revision_dto(db, ws, doc_id, version)`**
  - 约 240 行内联 DTO 构建+DB 查询整体迁移
  - 拆分为子方法：`_build_iteration_dict`、`_build_workflow_dict`、`_build_acl_dict`（复用 Batch 1 的统一方法）
- [ ] **`update_iteration` → `document_manager.py`**（~95 行）
- [ ] **`aborted_workflows` → `document_manager.py` 或 `workflow_manager.py`**
- [ ] **4 个 `inverse_*_link` → `document_manager.py`**
- [ ] **`update_doc_acl` → `document_manager.py`**
- [ ] **订阅/退订 4 函数 → `notification_manager.py`**
- [ ] **`remove_doc_file` / `rename_doc_file` → `document_manager.py`**
- [ ] **`_get_user_info`** → 合并到 `user_manager.py`（与 `products.py` 的 `_get_user_dto` 重复）
- [ ] 删除 `document.py` 中迁出的内联 helper 函数，路由端点改为调用对应 service 方法

### 主 agent Batch 3 收尾
- [ ] 部署 + smoke：文档 DTO 返回字段完整、update/checkout/checkin/subscribe 正常
- [ ] pytest 无新增 fail
- [ ] commit
...
- [ ] **`_ci_to_dict` DB 部分 → `product_structure.py`**
- [ ] **`ci_document_links` + `ci_document_links_wip` → `product_structure.py`**（~130 行）
- [ ] **`ci_paths` → `product_structure.py`**（~70 行）
- [ ] **`last_release` → `product_structure.py`**（~46 行）
- [ ] **`path_choices` / `versions_choices` → `product_structure.py`**
- [ ] **`path_to_path_links_types` / `path_to_path_links_detail` → `path_to_path_service.py`**
- [ ] **`bom` 的 flatten + DTO 构建部分 → `product_structure.py`**（递归逻辑保留在 service）
- [ ] **`filter_structure` admin check → 替换为 Batch 1 的 `Depends(require_workspace_admin)`**
- [ ] 删除 `products.py` 中迁出的内联 helper，端点改为委托 service

### 主 agent Batch 4 收尾
- [ ] 部署 + smoke：产品结构/CI 列表/路径/P2P 对拍 Payara
- [ ] pytest 无新增 fail
- [ ] commit
...
- [ ] **平台选项 → `platform_options_manager.py`**
  - `get_platform_options`、`put_platform_options`
- [ ] **admin stats 5 函数 → `platform_options_manager.py` 或新建 `admin_stats_service.py`**
- [ ] **`_require_admin` → 替换为 Batch 1 的 `Depends(require_admin)`**
- [ ] **`_account_to_dict` / `_workspace_to_dict`** — 轻量 DTO 映射保留在路由层（不超过 10 行纯字段映射）

### 主 agent Batch 5 收尾
- [ ] 部署 + smoke：admin 账户 CRUD/工作区 CRUD/平台选项/统计 无 500
- [ ] 特别验证 `delete_account` 级联完整性（DB 残留归零）
- [ ] pytest 无新增 fail
- [ ] commit
...
- [ ] `workspaces.py`：LOV CRUD 迁 `lov_manager.py`；stats 端点迁 `workspace_manager.py`
- [ ] `part.py`：`used_by_component`/`used_by_substitute` 迁 `product_manager.py`；publish/unpublish `db.commit()` 迁 service
- [ ] `tasks.py`：35 行 holder 解析逻辑迁 `task_manager.py`
...
- [ ] `user_groups.py`：内联 SQL + `db.commit()` 迁 `user_manager.py`
- [ ] `workspace_memberships.py`：内联 SQL + membership 校验迁 `user_manager.py`
- [ ] `document_templates.py`：update 的 attribute_templates 删插逻辑迁 `document_manager.py`

### 文件包 PKG-medium-3 → Subagent C
**Files:** `app/routers/users.py`、`app/routers/folders.py`、`app/routers/part_templates.py`

- [ ] `users.py`：内联 SQL + tag subscription 迁 `user_manager.py` / `notification_manager.py`
- [ ] `folders.py`：move_folder 路径替换+ORM 批量查询迁 `document_manager.py`
- [ ] `part_templates.py`：`_generate_from_mask` 等 ID 生成业务逻辑迁 `product_manager.py`
...
- [ ] 每个文件的内联 DB 操作迁至对应 service（有 service 的迁已有，无 service 的优先补齐）
- [ ] `layers.py`：确认 Java 侧 `LayerResource.java` 是否直接操作 EntityManager → 若是则保留 thin 实现；若 Java 有 `LayerManagerBean` 则在 tracker.csv 中补充并创建 service
- [ ] `change_common.py`：`_item_to_dict` 等共享 helpers（含内联 SQL）保持在一个文件，但改名定位为 service 层文件（或迁入一个新的 shared service）
...
- [ ] `cascade_action_manager.py` — 已在 Batch 2/4 部分激活，验证完整
- [ ] `public_entity_manager.py` — 检查 share/files 端点是否需调用；若 Java 侧只在 binary resource 端点使用，在对应 router 中接线
- [ ] `context_manager.py` — Java 中被 10+ 类注入，确认 Python 侧是否有等价需求
- [ ] `oauth_manager.py` — 如本期不实现 OAuth，标记为"对齐存根"并更新 tracker.csv
- [ ] `ondemand_converter.py` — 同上

### 文件包 PKG-service-activation-2 → Subagent B
低优先级/存根类：
- [ ] `activity_checker.py` — CDI 拦截器，Python 无等价机制，标记为存根
- [ ] `events/*`（12 个 Java 侧使用的） — CDI 事件系统，Python 不需要，标记为存根
- [ ] `listeners/*`（4 个） — CDI @Observes，Python 不需要，标记为存根
- [ ] `hooks/*`（3 个 webhook runner） — 是否需要在 `notifier.py` 中激活
- [ ] `storage/*`（3 个） — 加密存储，标记为存根或实现
- [ ] `gcm/gcm_sender.py` — Google Cloud Messaging，标记为存根
- [ ] `pending_conversions_cleaner.py` — `@Schedule` 定时器，需确认 Python 侧是否用 scheduler 替代
- [ ] `platform_health_manager.py` — 健康检查，确认 `platform.py` 是否已覆盖
- [ ] `documents/document_workflow_manager.py` / `products/part_workflow_manager.py` — 如无独立逻辑，标记合并到 `workflow_manager.py`

### 主 agent Batch 7 收尾
- [ ] 更新 tracker.csv：将所有存根 service 的状态从"已完成"改为"存根(对齐Java)"或"已激活"
- [ ] 更新 CHANGELOG / REMINDERS
- [ ] 最终全量 pytest + 对拍 Payara
- [ ] 将 checklist 第15条标记为"已审计+已制定修复计划"
