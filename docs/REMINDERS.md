...
> **⏸️ 待用户决策（批 7 effectivity 深化）**：B-6 EFFECTIVE_* 基线「best-effort」结论复核——`PartRevision.effectivities` relationship 确缺失、Effectivity `polymorphic_identity`("date_based") 与写入 dtype("D"/"S"/"L") 确不一致，但**这两处只影响 dead code**（`ProductBaselineService._create_effectivity_baseline` + config spec 未被任何 router 调用）。**API 实走** `ProductStructureService.create_baseline`→`_fill_effectivity_baselined_parts` **裸 SQL**（直接 JOIN partrevision_effectivity+effectivity 按 date/serial/lot 选 revision，无匹配退化 LATEST），不依赖上述 ORM 缺口。完整对齐两选项：(A 低风险，推荐) 保留裸 SQL 路径、按需硬化 serial/lot 字母数字比较；(B 高风险) 修 PartRevision.effectivities relationship + dtype + 将 API 改走 config-spec 路径——触碰高频核心模型、收益边际（裸 SQL 已选对）、GD50 无数据难验证。**未擅自改动核心模型，待用户拍板。**
>
> 以下为待修的最高优先级 P0：
