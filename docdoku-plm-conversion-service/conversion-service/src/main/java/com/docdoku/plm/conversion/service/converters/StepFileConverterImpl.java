    ...
    public ConversionResultProxy convert(final URI cadFileUri, final URI tmpDirUri)
        ...
        try {
            ...
            if (process.exitValue() == 0) {
                // Check for a companion .mtl file generated by the color-aware script
                List<Path> materials = new ArrayList<>();
                String baseName = tmpOBJFile.toString().replaceAll("\\.obj$", "");
                Path tmpMTLFile = Paths.get(baseName + ".mtl");
                if (Files.exists(tmpMTLFile) && Files.size(tmpMTLFile) > 0) {
                    materials.add(tmpMTLFile);
                    LOGGER.info("Color MTL file found: " + tmpMTLFile.toString());
                }
                return new ConversionResultProxy(tmpOBJFile, materials);
            } else {
