...
# docdoku-plm-api 构建生成的大量文件 & .gitignore 说明（2026-05-14 新增）

## 问题描述

运行 `mvn -f docdoku-plm-api/pom.xml -DskipTests install` 后，在以下两个目录中生成了上万个新文件：
- `docdoku-plm-api/docdoku-plm-api-base/`
- `docdoku-plm-api/docdoku-plm-api-js/`

---

## 这些文件是什么？

### `node_modules/` 目录（上万个文件的来源）

两个模块的 `pom.xml` 都配置了 `exec-maven-plugin`，在 Maven build 时调用 `npm run build`。  
`npm run build` 会先执行 `npm install`（分别由 `prebuild` 钩子触发），**自动下载并解压 `node_modules/` 目录**，包含：

| 模块 | npm 依赖 | 说明 |
|------|---------|------|
| `docdoku-plm-api-base` | `swagger-ui-dist`、`bestzip`、`concurrently` 等 | 生成 Swagger UI 文档，将 swagger.json 和 UI 文件打包成 zip |
| `docdoku-plm-api-js` | `grunt`、`grunt-browserify`、`grunt-cli` 等 | 使用 Grunt 将 Swagger 生成的 JS 代码打包为 `lib/docdoku-api.js` |

`node_modules/` 每个包含成百上千子文件，两个模块合计可达几万个文件，**这些都是构建产物，不应提交到 git**。
...
> ✅ 这条规则会自动忽略项目中所有 `node_modules/` 目录，包括当前和未来新增的模块。

**这个改动已经完成**，请执行 `git pull` 获取最新的 `.gitignore`。

---

## 关于 `package-lock.json` 无法加入 .gitignore 的问题

### 原因

`docdoku-plm-api/docdoku-plm-api-base/package-lock.json` 和 `docdoku-plm-api/docdoku-plm-api-js/package-lock.json` **已经被 git 跟踪（tracked）**。  
Git 的规则是：**已经被跟踪的文件，`.gitignore` 对其无效**。

### 应该怎么处理？

**推荐做法：保持 `package-lock.json` 被 git 跟踪（不忽略）。**

原因：
- `package-lock.json` 是 npm 的依赖锁定文件，保证在任何机器上 `npm install` 都能安装完全相同版本的依赖
- 提交它是 npm 官方推荐的做法
- 它不会产生大量文件（只有 1 个文件）

npm 在每次运行 `npm install` 时可能会更新 `package-lock.json`，这属于正常现象。

### 如果确实想忽略 `package-lock.json`（不推荐）

需要先从 git 跟踪中移除它，再加入 `.gitignore`：

```bash
# 停止跟踪（不会删除本地文件）
git rm --cached docdoku-plm-api/docdoku-plm-api-base/package-lock.json
git rm --cached docdoku-plm-api/docdoku-plm-api-js/package-lock.json

# 然后在 .gitignore 中加入：
# docdoku-plm-api/**/package-lock.json
```
...
# 为什么不重新编译 front 镜像？它能影响中文切换吗？（2026-05-14 新增）

## 问题分析

### 哪些修改是通过卷挂载生效的（不需要重建镜像）

目前 `docker-compose.yml` 中 `front` 服务挂载了：
...
| 挂载内容 | 是否已生效 |
|---------|----------|
| `nls/zh/` 目录下的所有中文翻译文件 | ✅ 已通过卷挂载生效 |
| `front/nginx.conf`（charset utf-8 + try_files） | ✅ 已通过卷挂载生效 |

### 哪些修改被锁在了镜像里（没有通过卷挂载）

以下前端 JS 文件和 HTML 模板的修改，**没有**对应的卷挂载，所以运行中的容器里使用的仍是 5 年前的旧版本：

| 文件 | 修改内容 | 当前状态 |
|------|---------|---------|
| `app/js/common-objects/contextResolver.js` | 语言不匹配时自动刷新页面 | ❌ 旧版本（镜像里） |
| `app/account-management/js/views/edit-account.js` | 保存语言后立即刷新 | ❌ 旧版本（镜像里） |
| `app/main/main.js` | 默认 locale 改为 zh | ❌ 旧版本（镜像里） |
| HTML 模板（硬编码英文） | 改为 i18n 模板变量 | ❌ 旧版本（镜像里） |

---

## 结论：重新编译 front 镜像能否影响中文切换？

**能，但影响有限。** 具体分析如下：

### 能改善的部分

1. **`contextResolver.js`**：修复了"locale 已是 zh 但 RequireJS 读取时还是 en"的 race condition。重建镜像后，语言切换会更加稳定、不需要两次刷新。
2. **`edit-account.js`**：修复了保存语言后需要手动刷新的问题。重建后保存语言会立即自动刷新。
3. **`main.js`**：登录页默认显示中文（针对全新浏览器）。
4. **硬编码英文模板**：部分界面（账号认证按钮、工作区描述字段等）会变成中文。

### 不能改善的部分

- **服务器端 API 返回的英文内容**：如错误消息、状态文字等，这部分是后端的问题，前端无论如何重建都不影响。
- **已经通过卷挂载生效的 NLS 翻译**：这部分已经正常工作了。
...
| 工具 | 版本要求 | 说明 |
|------|---------|------|
| Node.js | v6~v8（旧版！） | 新版 Node.js 与旧版 Bower/Grunt 不兼容 |
| npm | v3~v5 | 配合旧版 Node |
| Bower | ^1.8 | 已停止维护的前端包管理器 |
| Grunt CLI | ^1.2 | 构建工具 |
| Python 2 | （某些 npm 原生模块需要） | 已停止支持 |

这些工具已经相当老旧，在现代系统上配置它们往往会遇到各种兼容性问题。这也是为什么之前的指南（`Chinese-Language-Redeploy-Guide.md`）删除了"重建前端镜像"的方案 C。

---

## 更简单的替代方案：额外的卷挂载

如果想让 `contextResolver.js` 等 JS 文件的修改生效，**不需要重建镜像**，只需在 `docker-compose.yml` 中添加对应文件的卷挂载：

```yaml
# 在 front 服务的 volumes 下添加：
- ../docdoku-plm-front/app/js/common-objects/contextResolver.js:/usr/share/nginx/html/js/common-objects/contextResolver.js
- ../docdoku-plm-front/app/account-management/js/views/edit-account.js:/usr/share/nginx/html/account-management/js/views/edit-account.js
- ../docdoku-plm-front/app/main/main.js:/usr/share/nginx/html/main/main.js
```
...
> ⚠️ 注意：这种方式是挂载**未经 r.js 优化的源文件**，覆盖了镜像里的（可能已优化的）版本。大多数情况下可以正常工作，但如果镜像里使用了 r.js 把 contextResolver.js 打包进了其他 bundle，这种方式无效，仍需重建镜像。
...
| 方案 | 难度 | 能否解决中文切换问题 | 推荐？ |
|------|------|-------------------|-------|
| 继续使用现有卷挂载（NLS + nginx.conf） | 简单 | ⚠️ 部分（NLS 正常，JS 逻辑旧版） | ✅ 现状 |
